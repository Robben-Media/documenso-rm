...
export class LocalJobProvider extends BaseJobProvider {
  ...
  public getApiHandler(): (c: HonoContext) => Promise<Response | void> {
    return async (c: HonoContext) => {
      ...
      try {
        ...
        const retryAttempt = backgroundJob.retried + 1;
        const retryDelayMs = getJobRetryDelayMs(retryAttempt);

        console.warn(
          `[JOBS]: Delaying retry ${retryAttempt} for ${options.name} by ${retryDelayMs}ms after a transient failure`,
        );
  ...
  private async submitJobToEndpoint(options: {
    ...
    const { jobId, jobDefinitionId, data, isRetry, retryDelayMs } = options;
